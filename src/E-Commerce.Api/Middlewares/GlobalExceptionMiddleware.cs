...
/// into RFC 7807 ProblemDetails responses. Client cancellations and exceptions
/// after the response has started are re-thrown and left to ASP.NET Core.
/// </summary>
public sealed class GlobalExceptionMiddleware
{
    ...
    private async Task WriteErrorResponseAsync(HttpContext context, Exception exception)
    {
        ...
        // A 401 response must include a WWW-Authenticate header per RFC 7235.
        // The auth middleware adds it during challenge flow, but exceptions thrown
        // from handlers bypass that flow, so we set it explicitly.
        ...
        IDictionary<string, string[]>? errors)
    {
        ...
        var problem = new ProblemDetails
        {
            ...
            // For validation errors, the errors dictionary already carries details.
    ...
    /// <summary>
    /// Returns the value to expose as <c>traceId</c> in the ProblemDetails response.
    ///
    /// Priority order:
    /// 1. The correlation ID from <see cref="HttpContext.Items"/> (set by
    ///    CorrelationIdMiddleware and echoed in the X-Correlation-ID response header).
    ///    This ensures the ID quoted by clients in support requests matches both the
    ///    response header and the log scope.
    /// 2. The current OpenTelemetry trace ID, if a span is active.
    /// 3. ASP.NET Core's request identifier as a final fallback.
    /// </summary>
    private static string ResolveTraceId(HttpContext context)
    {
        if (context.Items.TryGetValue(ContextKeys.CorrelationId, out var value)
